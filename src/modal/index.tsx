...
const Modal = ({ onClick, cls = [], title, manage }: Props) => {
  ...
  if (manage) {
    console.log(manage[0]?.todayLastLogs[0]);
  }
                ...
                >
                  {manage.map((item, idx) => (
                    <S.ManageListItem key={idx}>
                      ...
                      >
                        <S.TitleSpan>
                          {item?.modifiedDate ? dayjs(item.modifiedDate).format('hh:mm:ss') : '출석기록 없음'}
                        </S.TitleSpan>
                      </div>
                      {['EIGTH', 'NINE', 'TEN', 'ELEVEN']
                        .map((key, index) => {
                          return item || { lastUpdated: null, position: key };
                        })
                        .map((item) => (
                          <>
                            <div
                            ...
                            >
                              <S.TitleSpan>
                                {item.todayLastLogs[idx]?.lastUpdated
                                  ? dayjs(item.todayLastLogs[idx].lastUpdated).format('hh:mm:ss')
                                  : '출석기록 없음'}
                              </S.TitleSpan>
                            </div>
                            <div
